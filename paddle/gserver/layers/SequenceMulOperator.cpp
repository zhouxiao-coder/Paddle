...
void SequenceMulOperator::forward() {
  ...
  size_t total_size = 0;
  out_->sequenceStartPositions = ICpuGpuVector::create(numSequences1 + 1, false);
  //ICpuGpuVector::resizeOrCreate(out_->sequenceStartPositions, numSequences1 + 1, false);
  ...
  for (size_t seqId = 0; seqId < numSequences1; ++seqId) {
    ...
    total_size += (startPositions1[seqId + 1] - startPositions1[seqId]) * \
      (startPositions2[seqId + 1] - startPositions2[seqId]);
    output_sequence_info[seqId + 1] = total_size;
    LOG(INFO) << "Joe: total_size= " << total_size;
  }  
  LOG(INFO) << "Joe: total_size= " << total_size;
  // update subsequence
  ICpuGpuVector::resizeOrCreate(out_->subSequenceStartPositions, total_size + 1, false);
  ...
  for (size_t seqId = 0; seqId < numSequences1; ++seqId) {
    // size_t endPos1 = seqId + 1 >= numSequences1 ? batch_size : startPositions1[seqId + 1];
    size_t endPos1 = startPositions1[seqId + 1];
    size_t seqLen1 = endPos1 - startPositions1[seqId];

    size_t endPos2 = startPositions2[seqId + 1];
    size_t seqLen2 = endPos2 - startPositions2[seqId];
    ...
    for (size_t r = 0; r < seqLen1; ++r) {
      auto current_row = tempMatrix->subRowMatrix(r, r + 1);
      current_row->print(os);
      LOG(INFO) << "current_row is " << r << ": " << os.str();
      current_row->softmax(*current_row);
      LOG(INFO) << "after softmax, current_row is " << r << ": " << os.str();
    } 
